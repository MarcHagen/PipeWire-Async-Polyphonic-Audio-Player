...
static bool create_pid_file(void) {
    ...
    if (xdg && xdg[0] != '\0') {
        if (snprintf(dir_path, sizeof(dir_path), "%s/%s", xdg, RUNTIME_SUBDIR) >= (int) sizeof(dir_path)) {
            log_error("PID dir path too long");
            return false;
        }
    } else {
        uid_t uid = getuid();
    ...
    // Ensure runtime subdirectory exists (0700), OK if it already exists
    if (mkdir(dir_path, 0700) < 0 && errno != EEXIST) {
        log_error("Failed to create runtime dir for PID: %s", dir_path);
        return false;
    }

    // Compose PID file path
    if (snprintf(pid_file_path, sizeof(pid_file_path), "%s/%s", dir_path, "papad.pid") >= (int) sizeof(pid_file_path)) {
        log_error("PID file path too long");
        return false;
    }
